...
events.push(function() {
	...
	***/
	...
	$('#category-left, #category-right, #graph-left, #graph-right, #time-period, #resolution, #graph-type, #invert, #start-date, #end-date, #start-time, #end-time').on('change', function() {
		// If one of the refresh intervals, or "Settings Change" options is selected.
		if (($( "#refresh-interval" ).val() >= 1000) || ($( "#refresh-interval" ).val() == -1)) {
			$("#chart-error").hide();
			draw_graph(getOptions());
	...
	$('#refresh-interval').on('change', function() {
		// If one of the refresh intervals options is selected.
		if ($( "#refresh-interval" ).val() >= 1000) {
			$("#chart-error").hide();
			draw_graph(getOptions());
		}
		refresh_graph_scheduler();
